...
import com.group4.clinicmanagement.service.admin.StaffForAdminService;
...
public class AdminController {
    ...
    private final StaffForAdminService staffForAdminService;
    private final UserService userService;
    private final DepartmentService departmentService;
    private final FeedbackService feedbackService;
    private final BillService billService;

    public AdminController(PatientForAdminService patientService, DoctorForAdminService doctorService, StaffForAdminService staffForAdminService, UserService userService, DepartmentService departmentService, FeedbackService feedbackService, BillService billService) {
        this.patientService = patientService;
        this.doctorService = doctorService;
        this.staffForAdminService = staffForAdminService;
    ...
    public String editPatientResult(@Valid @ModelAttribute(name = "patientDTO") PatientDTO dto,
                                    ...
                                    Model model) {
        if (patientService.isMailNoDuplicateForUpdPatient(dto.getEmail(), dto.getPatientId())) {
    ...
    public String addNewPatientResult(
        ...
        if (patientService.isUsernameDuplicateForNewPatient(dto.getUsername())) {
            bindingResult.rejectValue("username", "error.username", "Username already exists");
        }
        if (patientService.isPhoneNoDuplicateForNew(dto.getPhone())) {
            bindingResult.rejectValue("phone", "error.phone", "Phone already exists");
        }
    ...
    public String editDoctorResult(@Valid @ModelAttribute(name = "doctorDTO") DoctorDTO dto,
        ...
        if (staffForAdminService.isPhoneNoDuplicateForUpd(dto.getPhone(), dto.getDoctorId())) {
            bindingResult.rejectValue("phone", "error.phone", "Phone already exists");
        }
        if (staffForAdminService.isMailNoDuplicateForUpd(dto.getEmail(), dto.getUserId())) {
            bindingResult.rejectValue("email", "error.email", "Email already exists");
        }
        if (bindingResult.hasErrors()) {
            List<DepartmentDTO> patientDTOList = departmentService.findAll();
    ...
    public String addNewDoctorResult(
        ...
        if (staffForAdminService.isUsernameDuplicate(dto.getUsername())) {
            bindingResult.rejectValue("username", "error.username", "Username already exists");
        }
    ...
    public String showReceptionistList(Model model,
        ...
        try {
            ...
            Integer sizeC = Integer.parseInt(size);
            Pageable pageable = PageRequest.of(pageC, sizeC);
            Page<StaffDTO> ReceptionistDTOs = staffForAdminService.findAll(pageable, 1);
    ...
    public String showReceptionistById(@PathVariable(value = "id") String id, Model model, RedirectAttributes redirectAttributes) {
        try {
            Integer idC = Integer.parseInt(id);
            StaffDTO receptionistDTO = staffForAdminService.findById(idC, 1);
    ...
    public String editReceptionistResult(@Valid @ModelAttribute(name = "receptionistDTO") StaffDTO dto,
                                         BindingResult bindingResult,
                                         @RequestParam("avatar") MultipartFile avatar,
                                         RedirectAttributes redirectAttributes,
                                         Model model) {
        ...
        try {
            staffForAdminService.update(dto, avatar, 1);
            redirectAttributes.addFlashAttribute("successMessage",
    ...
    public String deleteReceptionistById(@ModelAttribute(name = "receptionistDTO") StaffDTO dto, RedirectAttributes redirectAttributes) {
        try {
            staffForAdminService.delete(dto, 1);
            redirectAttributes.addFlashAttribute("successMessage",
    ...
    public String addNewReceptionistResult(
        ...
        if (bindingResult.hasErrors()) {
            ...
            try {
                Integer receptionistId = staffForAdminService.newStaff(dto, avatar, 1);
                redirectAttributes.addFlashAttribute("successMessage",
